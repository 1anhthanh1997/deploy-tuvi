...
app.get("/api", (req, res) => {
  try {
    ...
    if (!namXemTieuVan) {
      let newThapNhiCung = [];
      for (let cung of data.thapNhiCung) {
        let listSao = cung.cungSao;
        let newListSao = listSao.filter((sao) => {
          return !sao.saoLuuNien;
        });
        cung.cungSao = newListSao;
        newThapNhiCung.push(cung);
      }
      data.thapNhiCung = newThapNhiCung;
    }
