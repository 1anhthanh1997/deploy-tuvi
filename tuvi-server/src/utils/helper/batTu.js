import { S2L, jdFromDate } from "./handleTime.js";
import {
  thienCan,
  diaChi,
  canChiNgay,
...
import tietData from "./tiet.json";
...
const buildDaiVanListFromMonthPillar = ({
  canThang,
  chiThang,
  amDuongNamNu,
  yearStartDecade,
}) => {
  if (!yearStartDecade) return [];
  return Array.from({ length: 9 }, (_, index) => {
    const decadeIndex = index + 1;
    const canIndex =
      amDuongNamNu === 1
    ...
    const chiIndex =
      amDuongNamNu === 1
        ...
        : dichCanChi(chiThang + 2, -decadeIndex, 12);
    const can = thienCan[canIndex].tenCan;
    const chi = diaChi[chiIndex].tenChi;
    const yearStart = yearStartDecade + index * 10;
    return {
      index: decadeIndex,
      can,
      chi,
...
const buildTieuVanList = (yearStart, timeZone = 7) => {
  if (!yearStart) return [];
  return Array.from({ length: 10 }, (_, index) => {
    const year = yearStart + index;
    const amLich = S2L(15, 5, year, timeZone);
    const canChi = ngayThangNamCanChi(amLich[0], amLich[1], amLich[2], false);
    const can = thienCan[canChi[1]].tenCan;
    const chi = diaChi[canChi[2]].tenChi;
    return {
      year,
      can,
      chi,
...
const getAgeStartDecadeFromTiet = (baseInfo, canNam, yearData) => {
  ...
  } else if (baseInfo.ngaySinh < tietInYear[nextTietIndex].startDate) {
    nextTietIndex = (nextTietIndex - 1 + 12) % 12;
  }
  const selectedTiet = tietInYear[nextTietIndex];
  const daysBetween = calculateDaysBetween(
    baseInfo.ngaySinh,
    baseInfo.thangSinh,
    baseInfo.namSinh,
    selectedTiet.startDate,
    selectedTiet.month,
    baseInfo.namSinh
  );
  return {
    ageStartDecade: Math.floor(daysBetween / 3),
    amDuongNamNu,
  };
};

const resolveDaiVanForYear = ({
  year,
  namSinh,
  yearStartDecade,
  amDuongNamNu,
  ...
  if (yearStartDecade == null || amDuongNamNu == null || !year) {
    return { decadeIndex: 0, yearStartDecade };
  }
  const tuoi = year - namSinh;
  const ageStartDecade = yearStartDecade - namSinh;
  if (tuoi < ageStartDecade) {
    return { decadeIndex: 0, yearStartDecade };
  }
  const decadeIndex = Math.floor((tuoi - ageStartDecade) / 10) + 1;
  const canDaiVan =
    amDuongNamNu === 1
      ? dichCanChi(canThang, decadeIndex, 10)
      : dichCanChi(canThang, -decadeIndex, 10);
  const chiDaiVan =
    amDuongNamNu === 1
      ? dichCanChi(chiThang + 2, decadeIndex, 12)
      : dichCanChi(chiThang + 2, -decadeIndex, 12);
  return { decadeIndex, canDaiVan, chiDaiVan, yearStartDecade };
};

const buildDaiVanPillar = ({
  canDaiVan,
  chiDaiVan,
  canNgay,
  decadeIndex,
  ...
  if (!(decadeIndex > 0 && canDaiVan != null && chiDaiVan != null)) {
    return {
      decadeIndex: decadeIndex ?? 0,
      yearStartDecade,
    };
  }
  return {
...
const getBaziData = (baseInfo, thapNhiCung, boTruGio) => {
  ...
  let onlyDecade = baseInfo.namXemDaiVan && !baseInfo.namXemTieuVan;
  ...
  const ageStartInfo = getAgeStartDecadeFromTiet(
    baseInfo,
    canChiAmLich[1],
    yearData
  );
  if (ageStartInfo) {
    amDuongNamNu = ageStartInfo.amDuongNamNu;
    yearStartDecade = ageStartInfo.ageStartDecade + baseInfo.namSinh - 1;
    daiVanList = buildDaiVanListFromMonthPillar({
      canThang,
      chiThang,
      amDuongNamNu,
      yearStartDecade,
  ...
  // Đại vận hiện tại luôn lấy theo năm lịch hiện tại (không phụ thuộc namXemDaiVan)
  const currentCalendarYear = new Date().getFullYear();
  let currentDaiVanResolved = resolveDaiVanForYear({
    year: currentCalendarYear,
    namSinh: baseInfo.namSinh,
    yearStartDecade,
    amDuongNamNu,
    canThang,
    chiThang,
  });
  if (currentDaiVanResolved.decadeIndex > 0 && yearStartDecade) {
    tieuVanList = buildTieuVanList(
      yearStartDecade + (currentDaiVanResolved.decadeIndex - 1) * 10,
      baseInfo.timeZone
    );
  }

  if (baseInfo.namXemTieuVan || baseInfo.namXemDaiVan) {
    if (!onlyDecade) {
      amLichVan = S2L(
    ...
    if (yearStartDecade != null && amDuongNamNu != null) {
      const selected = resolveDaiVanForYear({
        year: baseInfo.namXemDaiVan || baseInfo.namXemTieuVan,
        namSinh: baseInfo.namSinh,
        yearStartDecade,
        amDuongNamNu,
        canThang,
      ...
      decadeIndex = selected.decadeIndex;
      canDaiVan = selected.canDaiVan;
      chiDaiVan = selected.chiDaiVan;
    } else if (!boTruGio && thapNhiCung) {
      let tuoi =
        (baseInfo.namXemDaiVan || baseInfo.namXemTieuVan) - baseInfo.namSinh;
      let cungDaiVan = null;
  ...
  const currentDaiVan = buildDaiVanPillar({
    canDaiVan: currentDaiVanResolved.canDaiVan,
    chiDaiVan: currentDaiVanResolved.chiDaiVan,
    canNgay,
    decadeIndex: currentDaiVanResolved.decadeIndex,
    yearStartDecade,
  });
  ...
  return {
    ...
    // Before first đại vận: only metadata so template can show the message
    // without reading unset can/chi.
    // Đại vận theo năm xem (cho mục 3.x / tương tác động).
    daiVan:
      baseInfo.namXemDaiVan || baseInfo.namXemTieuVan
        ? buildDaiVanPillar({
            canDaiVan,
            chiDaiVan,
            canNgay,
        ...
        : undefined,
    // Đại vận theo năm lịch hiện tại (cho mục 1–2 phần III).
    currentDaiVan,
